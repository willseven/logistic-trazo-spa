"use client"
import { BackgroundBeams } from "@/components/ui/background-beams"
import { CardHover } from '../../../modules/cards/menu/stepcard';

export default function Menu ()  {
  return (
    <div className="min-h-screen w-full rounded-md bg-neutral-950 relative flex flex-col items-center justify-center antialiased">
  <div className="max-w-2xl mx-auto p-4 flex-1"> 
  <h1 className="relative z-10 text-5xl md:text-5xl lg:text-7xl bg-clip-text text-transparent bg-gradient-to-b from-neutral-200 to-neutral-600 text-center font-sans font-bold">
  Tipos
</h1>
    {/* <p className="text-neutral-500 max-w-lg mx-auto my-2 text-sm text-center relative z-10">
      Welcome to MailJet, the best transactional email service on the web. We provide reliable, scalable, and customizable email solutions for your business. Whether you&apos;re sending order confirmations, password reset emails, or promotional campaigns, MailJet has got you covered.
    </p> */}
    <CardHover/>
  </div>
  <BackgroundBeams />
</div>
  )
}
